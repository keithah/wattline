...
**Final verification evidence (2026-07-18):** the complete effective milestone range is `18c38822..HEAD`. The named pre-fix REDs included missing `TestProjectFiles`/bounded-wait helpers and missing `waitForSnapshotFanOutForTesting`; `testDirectConnectionPublishesBrokerReadinessBeforeConnectedPresentation`; `testReturnToScanWhileConnectedEventPublicationIsHeldCannotResurrectStateOrBrokerOwnership`; `testRestartReconnectOwnerAlonePresentsConnectedAfterBrokerReadiness`; `testRetryAfterRestartRecoveryTimeoutStartsFreshAttemptAndQuarantinesLateCompletion`; `testRestartReconnectDisconnectDuringLifecycleDeliveryRetriesWithoutStranding`; `testRestartDisconnectAfterBrokerSuccessResolutionCannotStrandMaintenance`; and the missing `waitForTelemetryPersistenceForTesting` completion seam. The final ownership review added non-vacuous REDs `testBrokerReconnectTimeoutInvalidatesOwnerBeforeLateConnectSuccess`, `testBrokerReconnectCallerCancellationInvalidatesOwnerBeforeLateConnectSuccess`, `testReversedBrokerReconnectRequestDeliveryKeepsNewestAttemptAsSoleOwner`, and `testDelayedReturnToScanDetachCannotDetachSameGenerationReplacementContext`. They proved that timeout/cancellation could previously allow a late owner to mutate presentation, reversed reconnect delivery could replace a newer owner, and a delayed generation-only detach could remove a replacement lifecycle. The original restart REDs were `testLateOldScopeDisconnectCannotTerminateRecoveredSession`, `testRestartAwaitsAsynchronousDisconnectDeliveredAfterWriteError`, `testRestartEntersRestartingAndReconnectsSamePeripheralWithoutScanning`, `testRestartRecoveryExposesRetryAtThirtySeconds`, `testRestartWaitsForScopedDisconnectThenReconnectsAtFifteenSecondsWithoutScan`, and `testWriteErrorAfterExpectedDisconnectStillRecovers`. The initial full-scheme UI REDs were `WattlineDemoUITests.testDemoModeDrivesEveryPhase1Surface`, `WattlineSystemSurfaceUITests.testDemoSettingsExposeSystemSurfacePreferencesAndNoTimers`, `WattlineSettingsUITests.testSettingsExposesRestartAndShutdownWithoutTimers`, and `WattlineSettingsUITests.testShutdownRequiresConfirmationAndCancelLeavesSettingsVisible`. Each behavioral assertion remains present.

Fresh package commands at the final source head passed WattlineCore 156/156, WattlineUI 26/26, and WattlineNetwork 97/97. Focused ownership coverage passed AppModelReconnect 49/49, SettingsLifecycle 19/19, and DeviceOperationBroker 16/16 (84/84 combined); Core QuirkRegression passed 10/10. Full real `Wattline` XCTest passed 157/157 with 0 failed/skipped (`/tmp/wattline-baseline-final-b4a9c98e-2.xcresult`); full real `WattlineWidgets` XCTest passed 157/157 with 0 failed/skipped (`/tmp/wattline-widgets-baseline-final-b4a9c98e-2.xcresult`). Destination: `Wattline-Tests-2`, iPhone 17e, UUID `74C1DA4D-7190-4497-AAD5-9EB140B3A96A`, iOS Simulator 26.5 / build 23F77.

The final Core/UI networking audit had no matches. The OTA/Timer audit had only six existing UI-test negative assertions/test names containing `Timers`; it had no `api.peakdo.ca`, `scheduledOnOff`, or `TimerRow` matches. `git diff --check` passed. A stability loop exposed `testTelemetryBecomesStaleAfterTenSeconds` reading state before the resumed fake-clock task completed its second actor hop; bounded state waits replaced one-shot scheduler yields in the deadline tests. The formerly flaky test then passed 100/100 isolated runs and the complete 156-test Core suite passed 20/20 consecutive runs. During verification, Xcode intermittently stalled before XCTest injection while materializing workers, and one erased-simulator run timed out loading Accessibility; terminating the stale Wattline host and running non-parallel on the warmed fallback simulator produced both clean result bundles. Empty-build-number, supported-platform, and debugger-version diagnostics remained non-fatal environment noise. No external check is required to establish this simulator baseline; real BLE restart/shutdown and signed-device background behavior remain hardware/external exit checks for their later feature milestones and were not claimed here. Milestone 2 has not started.
